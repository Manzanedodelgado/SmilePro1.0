...
const EVO_URL = import.meta.env.VITE_EVOLUTION_API_URL as string | undefined;
const EVO_KEY = import.meta.env.VITE_EVOLUTION_API_KEY as string | undefined;
const EVO_INSTANCE = import.meta.env.VITE_EVOLUTION_INSTANCE as string | undefined;

const CW_URL = import.meta.env.VITE_CHATWOOT_URL as string | undefined;
...
export const isEvolutionConfigured = (): boolean =>
    !!(EVO_URL && EVO_KEY && EVO_INSTANCE && EVO_URL !== 'https://your-evolution-api.com');

export const isChatwootConfigured = (): boolean =>
    !!(CW_URL && CW_TOKEN && CW_ACCOUNT && CW_URL !== 'https://your-chatwoot.com');
...
export const getQRCode = async (): Promise<string | null> => {
    if (!isEvolutionConfigured()) return null;
    try {
        const r = await fetch(`${EVO_URL}/instance/connect/${EVO_INSTANCE}`, {
            headers: evoHeaders(),
        });
        if (!r.ok) return null;
        const data = await r.json();
...
export const sendTemplateMessage = async (
    ...
    if (!isEvolutionConfigured()) return false;
    try {
        const r = await fetch(`${EVO_URL}/message/sendTemplate/${EVO_INSTANCE}`, {
            method: 'POST',
            headers: evoHeaders(),
            body: JSON.stringify({
                number: phone.replace(/\D/g, ''),
...
export const sendMediaMessage = async (
    ...
    try {
        ...
        const r = await fetch(`${EVO_URL}/message/${endpoint}/${EVO_INSTANCE}`, {
            method: 'POST',
            headers: evoHeaders(),
            body: JSON.stringify({ number: phone.replace(/\D/g, ''), mediaUrl, caption: caption ?? '' }),
        });
        return r.ok;
    } catch {
        return false;
