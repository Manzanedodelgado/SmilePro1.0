...
const SOAPEditor: React.FC<SOAPEditorProps> = ({
    ...
    const textAreaCls = "w-full px-3 py-2 bg-slate-50 border border-blue-200 rounded-lg text-sm text-slate-700 font-medium outline-none focus:border-blue-600/10 focus:ring-2 focus:ring-[rgb(30,64,175)]/10 resize-none transition-all placeholder:text-slate-300";
    ...
    const listenBg = listenState === 'listening'
        ...
        : listenState === 'analyzing'
            ? 'bg-amber-500'
            : 'bg-blue-700 hover:bg-blue-800/80';

    const tipoApp = selectedTto?.tipo_aplicacion ?? 'boca';

    return (
        <form onSubmit={handleSave} className="flex flex-col h-full bg-white rounded-xl border border-blue-200 shadow-sm overflow-hidden">
            {/* Header */}
            <div className="bg-slate-50 border-b border-blue-200 px-4 py-2 flex items-center justify-between flex-wrap gap-2 shrink-0">
                ...
                <div className="flex items-center gap-3">
                    <div className="flex items-center gap-1.5">
                        ...
                        <input
                            ...
                            className="text-xs font-bold text-slate-600 bg-transparent outline-none border-b border-dashed border-slate-300 focus:border-blue-600/10 transition-colors"
                        />
                    </div>
                    <div className="relative">
                        <select
                            ...
                            className="appearance-none bg-white border border-blue-200 text-xs font-bold rounded-lg pl-3 pr-7 py-1 outline-none text-slate-600 uppercase cursor-pointer hover:border-blue-600/10 transition-all"
                        >
            ...
            <div className="p-3 pb-2 flex-1 flex flex-col gap-3 min-h-0 overflow-y-auto custom-scrollbar">
                ...
                <div ref={ttoRef} className={`border rounded-lg p-2.5 transition-all ${!selectedTto && !initialData?.tratamiento_id
                    ...
                    <label className="text-[10px] font-black text-slate-900 uppercase tracking-wider flex items-center gap-1.5 mb-1.5">
                    ...
                    <div className="flex gap-2 items-start">
                        ...
                        <div className="flex-1 relative">
                            <div className="flex gap-1.5">
                                ...
                                <select
                                    ...
                                    className="appearance-none bg-slate-50 border border-blue-200 text-[10px] font-bold rounded-lg pl-2 pr-5 py-1.5 outline-none text-slate-600 cursor-pointer hover:border-blue-600/10 transition-all w-28 shrink-0"
                                >
                                ...
                                <div className="relative flex-1">
                                    ...
                                    <input
                                        ...
                                        className="w-full pl-7 pr-3 py-1.5 bg-slate-50 border border-blue-200 rounded-lg text-xs font-medium text-slate-700 outline-none focus:border-blue-600/10 focus:ring-2 focus:ring-[rgb(30,64,175)]/10 transition-all placeholder:text-slate-300"
                                    />
                                    {selectedTto && (
                            ...
                            {showTtoDropdown && ttoResults.length > 0 && (
                                <div className="absolute z-50 top-full mt-1 w-full bg-white border border-blue-200 rounded-lg shadow-lg max-h-48 overflow-y-auto">
                                    {ttoResults.map(tto => (
                                        <button
                        ...
                        {selectedTto && tipoApp === 'pieza' && (
                            <select
                                ...
                                className="appearance-none bg-slate-50 border border-blue-200 text-xs font-bold rounded-lg pl-2 pr-5 py-1.5 outline-none text-slate-600 w-20 shrink-0"
                            >
                        ...
                        {selectedTto && tipoApp === 'arcada' && (
                            <select
                                ...
                                className="appearance-none bg-slate-50 border border-blue-200 text-xs font-bold rounded-lg pl-2 pr-5 py-1.5 outline-none text-slate-600 w-24 shrink-0"
                            >
                ...
                <div className="grid grid-cols-2 gap-3 flex-1 min-h-0">
                    ...
                    <div className="flex flex-col min-h-0">
                        <div className="flex items-center justify-between mb-1">
                            ...
                            <div className="flex items-center gap-1 bg-slate-50 border border-blue-200 rounded-full px-2 py-0.5">
                                <span className="text-[9px] font-bold text-slate-500">EVA:</span>
                                <input
                                    type="number" min={0} max={10}
                                    value={nota.eva}
                                    onChange={e => setNota({ ...nota, eva: parseInt(e.target.value) || 0 })}
                                    className="w-5 bg-transparent text-[10px] font-black text-slate-900 outline-none text-center"
                                />
                    ...
                    {/* P */}
                    <div className="flex flex-col min-h-0">
                        <label className="text-[10px] font-black text-slate-900 uppercase tracking-wider flex items-center gap-1.5 mb-1">
                            <span className="w-2 h-2 rounded-full bg-primary" /> P — Plan
                        </label>
                ...
                {/* Footer */}
                <div className="flex items-center justify-between pt-2 border-t border-blue-100 shrink-0 mt-auto">
                    ...
                    <div className="flex items-center gap-2">
                        {onCancel && (
                            <button type="button" onClick={onCancel}
                                className="px-3 py-1.5 border border-blue-200 rounded-lg text-[9px] font-black uppercase text-slate-500 hover:bg-slate-50 transition-all">
                                Cancelar
                            </button>
                        )}
                        <button
                            type="submit" disabled={saving}
                            className="flex items-center gap-1.5 bg-gradient-to-r from-blue-600 to-blue-800 text-white px-4 py-1.5 rounded-lg font-black uppercase text-[10px] tracking-wider shadow-md hover:bg-blue-900 active:scale-95 transition-all disabled:opacity-60"
                        >
                            {saving
