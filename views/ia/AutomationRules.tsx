...
export type AutoCategory = 'recordatorio' | 'seguimiento' | 'recaptacion' | 'pago' | 'documento' | 'urgencia' | 'clinico' | 'gestion';
...
const GROUPS: { key: AutoCategory; label: string; border: string; bg: string; ids: string[] }[] = [
    ...
    { key: 'seguimiento', label: '💙 Seguimiento Clínico', border: 'border-rose-200', bg: 'bg-rose-50/40', ids: ['post-basic', 'post-surgery', 'post-implant-2m', 'post-recs', 'doctor-alert', 'doctor-followup'] },
    { key: 'recaptacion', label: '✨ Recaptación y Fidelización', border: 'border-emerald-200', bg: 'bg-emerald-50/40', ids: ['recall-6m', 'recall-annual', 'review-request', 'birthday'] },
    ...
    { key: 'gestion', label: '⚙️ Gestión de Agenda', border: 'border-blue-200', bg: 'bg-slate-50/40', ids: ['no-show', 'confirm-status'] },
    { key: 'urgencia', label: '🚨 Urgencias', border: 'border-red-200', bg: 'bg-red-50/40', ids: ['urgency-auto'] },
];
...
export const INITIAL_AUTOMATIONS: Automation[] = [
    ...
    mk('first-visit-questionnaire', 'Cuestionario Primera Visita', 'Para nuevos pacientes: anamnesis automática.', 'Primera cita para paciente nuevo', 'whatsapp', 'documento', true, 201, 87, '72h antes primera visita', '📋 ¡Bienvenido/a a Rubio García Dental, {{nombre}}! Completa este cuestionario de salud en 3 minutos: {{enlace_cuestionario}}'),
...
const ConfigPanel: React.FC<{ a: Automation; onSave: (cfg: Automation['config']) => void; onClose: () => void }> = ({ a, onSave, onClose }) => {
    ...
    return (
        <div className="border-t border-blue-100 bg-slate-50/60 px-4 py-4 space-y-4 animate-in fade-in duration-150">
            <div className="flex items-center justify-between mb-1">
                <span className="text-[11px] font-black text-slate-900 uppercase tracking-widest flex items-center gap-1.5"><Settings2 className="w-3.5 h-3.5 text-primary" />Configuración</span>
            ...
            <div className="grid grid-cols-2 gap-3">
                <div>
                    <label className="block text-[9px] font-black text-slate-400 uppercase tracking-wider mb-1">Demora tras trigger</label>
                    <div className="flex gap-1.5">
                        <input type="number" min={0} value={cfg.delayValue} onChange={e => setCfg(p => ({ ...p, delayValue: +e.target.value }))}
                            className="w-16 bg-white border border-blue-200 rounded-lg px-2 py-1.5 text-[12px] font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-primary/20" />
                        <select value={cfg.delayUnit} onChange={e => setCfg(p => ({ ...p, delayUnit: e.target.value }))}
                            className="flex-1 bg-white border border-blue-200 rounded-lg px-2 py-1.5 text-[12px] font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-primary/20">
                            {UNITS.map(u => <option key={u}>{u}</option>)}
                        </select>
                    </div>
                </div>
                <div>
                    <label className="block text-[9px] font-black text-slate-400 uppercase tracking-wider mb-1">Horario de envío</label>
                    <select value={cfg.schedule} onChange={e => setCfg(p => ({ ...p, schedule: e.target.value }))}
                        className="w-full bg-white border border-blue-200 rounded-lg px-2 py-1.5 text-[12px] font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-primary/20">
            ...
            {/* Channel */}
            <div>
                <label className="block text-[9px] font-black text-slate-400 uppercase tracking-wider mb-1">Canal</label>
                <div className="flex gap-1.5 flex-wrap">
                    {Object.entries(CH).map(([key, v]) => (
                        <button key={key} onClick={() => setCfg(p => ({ ...p, channel: key }))}
                            className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-[10px] font-bold transition-all ${cfg.channel === key ? 'bg-[#0056b3] text-white' : 'bg-white border border-blue-200 text-slate-500 hover:border-[#0056b3]/30'}`}>
            ...
            {/* Conditions */}
            <div>
                <label className="block text-[9px] font-black text-slate-400 uppercase tracking-wider mb-1">Condición adicional (opcional)</label>
                <input value={cfg.conditions} onChange={e => setCfg(p => ({ ...p, conditions: e.target.value }))}
                    ...
                    className="w-full bg-white border border-blue-200 rounded-lg px-3 py-1.5 text-[11px] focus:outline-none focus:ring-2 focus:ring-primary/20" />
            </div>

            {/* Message template */}
            <div>
                <div className="flex items-center justify-between mb-1">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-wider">Plantilla del mensaje</label>
                    <div className="flex gap-1 flex-wrap justify-end">
                        {VARS.map(v => (
                            <button key={v} onClick={() => setCfg(p => ({ ...p, message: p.message + ' ' + v }))}
                                className="text-[9px] font-black px-1.5 py-0.5 bg-[#0056b3]/10 text-primary rounded border border-[#0056b3]/20 hover:bg-[#0056b3]/20 transition-all font-mono">
                                {v}
                            </button>
                        ))}
                    </div>
                ...
                <textarea value={cfg.message} onChange={e => setCfg(p => ({ ...p, message: e.target.value }))} rows={4}
                    className="w-full bg-white border border-blue-200 rounded-xl px-3 py-2 text-[11px] text-slate-700 font-mono focus:outline-none focus:ring-2 focus:ring-primary/20 resize-none" />
                <p className="text-[9px] text-slate-400 mt-1">Usa las variables de arriba. Serán reemplazadas con los datos reales del paciente al enviar.</p>
            </div>

            <button onClick={save} className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all ${saved ? 'bg-emerald-500 text-white' : 'bg-[#0056b3] text-white hover:bg-[#004494]'}`}>
...
const Card: React.FC<{ a: Automation; expanded: boolean; configOpen: boolean; onToggle: () => void; onExpand: () => void; onConfig: () => void; onSaveConfig: (cfg: Automation['config']) => void }> =
    ({ a, expanded, configOpen, onToggle, onExpand, onConfig, onSaveConfig }) => {
        ...
        return (
            <div className={`bg-white rounded-xl border transition-all ${a.active ? 'border-blue-200' : 'border-blue-100 opacity-60'} ${expanded || configOpen ? 'shadow-md ring-1 ring-[#0056b3]/10' : 'hover:shadow-sm'}`}>
                <div className="flex items-center gap-3 px-4 py-3 cursor-pointer" onClick={onExpand}>
                    <div className="flex-1 min-w-0">
                        <span className="text-[12px] font-black text-slate-900 block truncate">{a.name}</span>
                        <div className="flex items-center gap-3 text-[10px] text-slate-400 mt-0.5">
                            ...
                            <span className="text-primary font-black">{a.successRate}% éxito</span>
                        </div>
                    </div>
                    <button onClick={e => { e.stopPropagation(); onToggle(); }}>
                        {a.active ? <ToggleRight className="w-7 h-7 text-primary" /> : <ToggleLeft className="w-7 h-7 text-slate-300" />}
                    </button>
                    {expanded ? <ChevronUp className="w-4 h-4 text-slate-300" /> : <ChevronDown className="w-4 h-4 text-slate-300" />}
                </div>

                {expanded && !configOpen && (
                    <div className="border-t border-blue-100 px-4 pb-4 pt-3 space-y-3 animate-in fade-in duration-150">
                        ...
                        <div className="bg-[#ffe4e6] border border-[#fecdd3] rounded-xl p-3">
                            <p className="text-[9px] font-black text-rose-700 uppercase tracking-wider mb-1.5">Plantilla actual</p>
                            <p className="text-[11px] text-rose-900 font-mono leading-relaxed">{a.config.message}</p>
                        </div>
                        <div className="grid grid-cols-3 gap-2 text-[10px]">
                            <div className="bg-slate-50 rounded-lg p-2 border border-blue-100">
                                <p className="text-slate-400 font-bold uppercase text-[9px]">Demora</p>
                                <p className="font-black text-slate-900">{a.config.delayValue} {a.config.delayUnit}</p>
                            </div>
                            <div className="bg-slate-50 rounded-lg p-2 border border-blue-100">
                                <p className="text-slate-400 font-bold uppercase text-[9px]">Horario</p>
                                <p className="font-black text-slate-900 truncate">{a.config.schedule}</p>
                            </div>
                            <div className="bg-slate-50 rounded-lg p-2 border border-blue-100">
                                <p className="text-slate-400 font-bold uppercase text-[9px]">Envíos</p>
                                <p className="font-black text-slate-900">{a.executions}</p>
                            </div>
                        </div>
                        <div className="flex gap-2">
                            <button onClick={e => { e.stopPropagation(); onConfig(); }} className="flex items-center gap-1 px-3 py-1.5 text-[10px] font-black uppercase bg-[#0056b3] text-white rounded-lg hover:bg-[#004494] transition-all">
                                <Settings2 className="w-3 h-3" />Configurar
                            </button>
                            <button className="flex items-center gap-1 px-3 py-1.5 text-[10px] font-black uppercase border border-[#0056b3]/20 text-primary rounded-lg hover:bg-[#0056b3]/5 transition-all">
...
export const AutomationRules: React.FC = () => {
    ...
    const toggle = (id: string) => setAutomations(p => p.map(a => a.id === id ? { ...a, active: !a.active } : a));
    ...
    const saveConfig = (id: string, cfg: Automation['config']) => setAutomations(p => p.map(a => a.id === id ? { ...a, config: cfg } : a));
    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center gap-3">
                <span className="text-[10px] font-black text-slate-400 uppercase">Motor automatización</span>
                <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                <span className="text-[10px] font-black text-emerald-600">{activeCount}/{automations.length} activas</span>
            </div>
                ...
                return (
                    <div key={g.key} className={`rounded-2xl border ${g.border} ${g.bg} overflow-hidden`}>
                        <div className="flex items-center justify-between px-4 py-2.5 cursor-pointer" onClick={() => toggleGroup(g.key)}>
                            <div className="flex items-center gap-3">
                                <span className="text-[12px] font-black text-slate-900">{g.label}</span>
                                <span className="text-[10px] font-bold text-slate-400">{activeInGroup}/{items.length}</span>
                            </div>
